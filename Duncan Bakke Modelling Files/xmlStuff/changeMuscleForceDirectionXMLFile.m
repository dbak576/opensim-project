function err = changeMuscleForceDirectionXMLFile(filename,title,timerange,model,directory)
...
CoordinateFileName = strcat(directory,'\',model,'\',title,'\',title,'IKResults.mot');
CoordinateFileChild.getFirstChild.setData(CoordinateFileName);

externalLoadsFile = strcat(directory,'\',model,'\',title,'\',title,'ExternalLoads.xml');
