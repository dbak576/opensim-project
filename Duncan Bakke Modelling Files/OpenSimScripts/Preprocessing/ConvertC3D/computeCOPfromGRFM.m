function actionGRFTz_FP =  computeCOPfromGRFM(analogGRFM_FP, fpInfo, analogRate)
% USAGE:    computeCOPfromGRFM(analogGRFM_FP, fpInfo, analogRate)
% Purpose:  Computes the center of pressure (COP) and vertical torque (Tz)
%           from the 'action' forces and moments (Fx, Fy, Fz, Mx, My, Mz) 
%           stored in analogGRFM_FP, in the FP coordinate systems, for
...
% Get num of foot strikes on force-plates
nHits = length(fpInfo);
for I = 1:nHits,
    actionGRFM_FP(I).Fx = analogGRFM_FP(:,6*I-5);
    actionGRFM_FP(I).Fy = analogGRFM_FP(:,6*I-4);
    actionGRFM_FP(I).Fz = analogGRFM_FP(:,6*I-3);
    actionGRFM_FP(I).Mx = analogGRFM_FP(:,6*I-2);
    actionGRFM_FP(I).My = analogGRFM_FP(:,6*I-1);
    actionGRFM_FP(I).Mz = analogGRFM_FP(:,6*I-0); 
end

 

% Get number of analog frames in 'simulateable' segment.
nAnalogFrames = length(analogGRFM_FP);
...
for fpHitNum = 1:nHits
    ...
    threshold = 0.01*max(abs(actionGRFM_FP(fpHitNum).Fz));
    contactIndices = find(abs(actionGRFM_FP(fpHitNum).Fz) > threshold);
