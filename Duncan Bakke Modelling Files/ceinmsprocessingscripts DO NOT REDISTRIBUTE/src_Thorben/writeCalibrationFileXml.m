...
function writeCalibrationFileXml(templateCalXML, trialSet, jointsForCalibration, fileOut, pref)
    ...
    prefDef.tendonType = 'stiff'; %'equilibriumElastic' 'integrationElastic'
    prefDef.activationType='exponential'; %'piecewise'
    prefDef.parameterShareType = 'single'; %'global' 
    prefDef.objectiveFunction = 'minimizeTorqueError'; %'torqueErrorAndSumKneeContactForces'
    ...
    if useActivationScale == true
        tree.calibrationSteps.step.parameterSet.parameter(7).name = 'activationScale';
        tree.calibrationSteps.step.parameterSet.parameter(7).absolute = struct('range',activationScaleRange);
        tree.calibrationSteps.step.parameterSet.parameter(7).muscleGroups = tree.calibrationSteps.step.parameterSet.parameter(6).muscleGroups;
    end
    ...
    elseif strcmp(prefDef.objectiveFunction, 'minimizeTorqueErrorAndActivation')
		tree.calibrationSteps.step.objectiveFunction.(prefDef.objectiveFunction)= struct;
	end
    ...
    tree.calibrationSteps.step.parameterSet.parameter(6,1).absolute.range = '0.9 1.1';
